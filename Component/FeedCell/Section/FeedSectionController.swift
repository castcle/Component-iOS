...
public class FeedSectionController: ListSectionController {
    var feed: Feed?
    
    public var delegate: FeedSectionControllerDelegate?
...
extension FeedSectionController {
    ...
    public override func cellForItem(at index: Int) -> UICollectionViewCell {
        switch index {
        case FeedCellSection.header.rawValue:
            let cell = collectionContext?.dequeueReusableCell(withNibName: ComponentNibVars.CollectionViewCell.headerFeed, bundle: ConfigBundle.component, for: self, at: index) as? HeaderFeedCell
            cell?.backgroundColor = UIColor.Asset.darkGray
            cell?.delegate = self
            cell?.feed = self.feed
            return cell ?? HeaderFeedCell()
        case FeedCellSection.footer.rawValue:
            let cell = collectionContext?.dequeueReusableCell(withNibName: ComponentNibVars.CollectionViewCell.footerFeed, bundle: ConfigBundle.component, for: self, at: index) as? FooterFeedCell
            cell?.backgroundColor = UIColor.Asset.darkGray
            cell?.delegate = self
            cell?.feed = self.feed
        ...
        case FeedCellSection.content.rawValue:
            if self.feed?.feedPayload.feedDisplayType == .postImageX1 {
                let cell = collectionContext?.dequeueReusableCell(withNibName: ComponentNibVars.CollectionViewCell.imageX1Cell, bundle: ConfigBundle.component, for: self, at: index) as? ImageX1Cell
                cell?.backgroundColor = UIColor.Asset.darkGray
                cell?.feed = self.feed
                return cell ?? TextCell()
            } else if self.feed?.feedPayload.feedDisplayType == .postImageX2 {
                let cell = collectionContext?.dequeueReusableCell(withNibName: ComponentNibVars.CollectionViewCell.imageX2Cell, bundle: ConfigBundle.component, for: self, at: index) as? ImageX2Cell
                cell?.backgroundColor = UIColor.Asset.darkGray
                cell?.feed = self.feed
                return cell ?? ImageX2Cell()
            } else if self.feed?.feedPayload.feedDisplayType == .postImageX3 {
                let cell = collectionContext?.dequeueReusableCell(withNibName: ComponentNibVars.CollectionViewCell.imageX3Cell, bundle: ConfigBundle.component, for: self, at: index) as? ImageX3Cell
                cell?.backgroundColor = UIColor.Asset.darkGray
                cell?.feed = self.feed
                return cell ?? ImageX3Cell()
            } else if self.feed?.feedPayload.feedDisplayType == .postImageXMore {
                let cell = collectionContext?.dequeueReusableCell(withNibName: ComponentNibVars.CollectionViewCell.imageXMoreCell, bundle: ConfigBundle.component, for: self, at: index) as? ImageXMoreCell
                cell?.backgroundColor = UIColor.Asset.darkGray
                cell?.feed = self.feed
                return cell ?? ImageXMoreCell()
            } else if self.feed?.feedPayload.feedDisplayType == .postText {
                let cell = collectionContext?.dequeueReusableCell(withNibName: ComponentNibVars.CollectionViewCell.postText, bundle: ConfigBundle.component, for: self, at: index) as? TextCell
                cell?.backgroundColor = UIColor.Asset.darkGray
                cell?.feed = self.feed
                return cell ?? TextCell()
            } else if self.feed?.feedPayload.feedDisplayType == .postLink || self.feed?.feedPayload.feedDisplayType == .postYoutube {
                let cell = collectionContext?.dequeueReusableCell(withNibName: ComponentNibVars.CollectionViewCell.postTextLinkCell, bundle: ConfigBundle.component, for: self, at: index) as? TextLinkCell
                cell?.backgroundColor = UIColor.Asset.darkGray
                cell?.feed = self.feed
                return cell ?? TextLinkCell()
            } else if self.feed?.feedPayload.feedDisplayType == .blogImage {
                let cell = collectionContext?.dequeueReusableCell(withNibName: ComponentNibVars.CollectionViewCell.blogCell, bundle: ConfigBundle.component, for: self, at: index) as? BlogCell
                cell?.backgroundColor = UIColor.Asset.darkGraphiteBlue
                cell?.feed = self.feed
                return cell ?? BlogCell()
            } else if self.feed?.feedPayload.feedDisplayType == .blogNoImage {
                let cell = collectionContext?.dequeueReusableCell(withNibName: ComponentNibVars.CollectionViewCell.blogNoImageCell, bundle: ConfigBundle.component, for: self, at: index) as? BlogNoImageCell
                cell?.backgroundColor = UIColor.Asset.darkGraphiteBlue
                cell?.feed = self.feed
...
extension FeedSectionController: HeaderFeedCellDelegate, FooterFeedCellDelegate {
    func didTabProfile() {
        self.delegate?.didTabProfile()
    }
